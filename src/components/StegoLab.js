...
export class StegoLab {
  constructor(container) {
    this.container = container;
    this.selectedBitPlane = 0;
    this.cipherShift = 13;
  ...
  render() {
    this.container.innerHTML = `
      <div class="pixel-matrix-card" style="padding: 36px; border-radius: 24px;">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px; flex-wrap: wrap; gap: 16px;">
          <div>
            ...
            <h3 class="font-pixel" style="font-size: 1.15rem; color: var(--accent-cyan); letter-spacing: 0.05em;">PIXEL MATRIX // LSB EXTRACTOR</h3>
          </div>
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 28px; align-items: center;">
          ...
          <div style="text-align: center;">
            <canvas id="stego-canvas" width="280" height="240" style="background: #020409; border: 1px solid rgba(0, 245, 255, 0.3); border-radius: 12px; width: 100%; max-width: 320px;"></canvas>
            <div class="font-mono" style="font-size: 0.75rem; color: var(--text-muted); margin-top: 8px;">
              [RESOLUTION: 64x64 BITPLANE] • [LSB NOISE SPECTROGRAM]
            </div>
          </div>

          <!-- Decoded Data & Cryptographic Rotor -->
          <div class="neu-inset" style="padding: 24px; border-radius: 16px;">
            ...
            <div id="hex-output" class="font-mono" style="font-size: 0.78rem; color: var(--text-secondary); background: rgba(0,0,0,0.6); padding: 12px; border-radius: 8px; height: 100px; overflow-y: auto; line-height: 1.5; border: 1px solid rgba(255,255,255,0.05);">
            ...
            <div id="plaintext-result" class="font-mono" style="margin-top: 14px; padding: 10px 14px; background: rgba(0, 245, 255, 0.08); border-left: 3px solid var(--accent-cyan); font-size: 0.82rem; color: #ffffff; word-break: break-all; border-radius: 4px;">
  ...
  initCanvasMatrix() {
    ...
    for (let i = 0; i < data.length; i += 4) {
      ...
      data[i] = bit === 0 ? 0 : val * 0.5;
      data[i + 1] = val;
      data[i + 2] = bit === 0 ? val : val;
      data[i + 3] = 255;
    }
